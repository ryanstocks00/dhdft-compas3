#!/usr/bin/env python3
"""
orca_property_to_csv.py

Scan a folder for ORCA *.property.txt files and extract key energy terms
into a single CSV.

Fields captured (when present):
- filename
- version, progname, status
- scf_energy_hartree
- dft_eexchange_hartree, dft_ecorr_hartree, dft_exc_hartree, dft_finalen_hartree
- mp2_ref_hartree, mp2_corr_hartree, mp2_total_hartree
- vdw_correction_hartree  (e.g., D3/D4 or general VdW correction reported by ORCA)
- total_energy_hartree    (from $Calculation_Info)
- mult, charge, natoms

Usage:
  python orca_property_to_csv.py /path/to/folder -o energies.csv
"""
...
# Regexes to detect sections and parse &KEY lines
SECTION_START = re.compile(r'^\s*\$(\S+)')
SECTION_END = re.compile(r'^\s*\$End\s*$')

# Example line forms:
#   &SCF_ENERGY [&Type "Double"]      -9.1957094360515430e+02
#   &FINALEN [&Type "Double"]      -9.1957094360515430e+02  "No Van der Waals correction"
#   &VERSION [&Type "String"] "6.0.1"
# We capture the first number or string literal on the line if present.
NUM_RE = r'([+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?)'
STR_RE = r'"([^"]*)"'
...
def extract_fields(sections: Dict[str, Dict[str, str]]) -> Dict[str, float]:
    ...
    If a field is missing, it will be set to NaN.
    """
    out: Dict[str, float] = {}
...
def main():
    parser = argparse.ArgumentParser(description="Extract ORCA .property.txt energies to CSV")
    parser.add_argument("folder", help="Folder containing *.property.txt files")
    parser.add_argument("-o", "--output", default="energies.csv", help="Output CSV filename")
    ...
    fieldnames = preferred_order + [k for k in sorted(all_keys) if k not in preferred_order]
